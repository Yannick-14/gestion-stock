...
public class StockManager {
    ...
    // Process to make an movement stock with mode exit (Out movement)
    public List<StockMovement> processExitRequest(StockMovement exitRequest) throws Exception {
        ...
        String method = article.getStockManagementMethod().getNameMethod().toLowerCase();
        String transactionRef = exitRequest.getTransactionRef() != null ? exitRequest.getTransactionRef() : generateRef();
    ...
    private List<StockMovement> processCUMP(List<Batch> batches, Article article, StockMovement exitRequest, String transactionRef) {
        double cump = calculateTotalValue(batches) / (double) batches.stream().mapToInt(b -> b.getQuantity()).sum();
        StockMovement movement = new StockMovement(article, exitRequest.getTypeStockMovement(), exitRequest.getQuantity(), cump);
        movement.setTransactionRef(transactionRef);
        return List.of(movement);
    }

    private List<StockMovement> processFIFOorLIFO(List<Batch> batches, Article article, StockMovement exitRequest, String transactionRef, boolean isLIFO) {
        ...
        for (Batch batch : batches) {
            ...
            StockMovement movement = new StockMovement(article, exitRequest.getTypeStockMovement(), taken, batch.getPrice());
            movement.setTransactionRef(transactionRef);
    ...
    // Calculer l'etat de stock de prix totale ici
